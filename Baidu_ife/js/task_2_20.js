/**
 * Created by FengSi on 2017/01/10 0010.
 */
//事件绑定函数，兼容浏览器差异
...
window.onload = function() {
        ...
        search: function (target) {
            ...
            each(this.str, function (item) {
                if (item.indexOf(target) >= 0)
                    str += "<div style='background-color: #86C7FF;'>" + item + "</div>";
                else
                    str += ("<div>" + item + "</div>");
            })
            container.innerHTML = str;
    ...
    addEvent(buttonList[1], "click", function() {
        queue.rightPush(txtinput.value);
    });
    ...
    addEvent(buttonList[5], "click", function () {
        queue.search(buttonList[4].value);
    })
}